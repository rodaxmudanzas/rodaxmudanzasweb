...
function renderFotosDrawer(urlsFotosString) {
    // Buscamos tanto el contenedor del resumen (si existe) como el de la pestaña de Fotos
    const contenedorResumen = document.getElementById("drawerFotos");
    const contenedorPestaña = document.getElementById("drawerFotosFull");
    
    // Elegimos el contenedor que esté disponible en el HTML actual de la página
    const contenedorDestino = contenedorPestaña || contenedorResumen;
    
    if (!contenedorDestino) {
        console.error("No se encontró ningún contenedor válido para las fotos en el HTML ('drawerFotosFull' o 'drawerFotos')");
        return;
    }

    contenedorDestino.innerHTML = ""; // Limpiar fotos de cargas o expedientes anteriores

    // Si el cliente no subió imágenes o viene vacío, mostramos un aviso limpio centrado
    if (!urlsFotosString || urlsFotosString.trim() === "") {
        contenedorDestino.innerHTML = `<div class="text-center text-slate-400 text-xs py-6 italic w-full">El cliente no adjuntó fotografías para este servicio.</div>`;
        return;
    }

    // Separamos la cadena de texto por comas (,) o barras verticales ( | ) para obtener los enlaces individuales
    const arrayFotos = urlsFotosString.split(/,|\s*\|\s*/);
    
    // Creamos la cuadrícula Responsive: 3 columnas en móviles pequeños y 4 columnas en tablets/pantallas grandes
    ...
    contenedorDestino.appendChild(gridFotos);

    // Inyectar el Visor Modal flotante en el documento (solo si no se ha creado antes)
    if (!document.getElementById("rodaxFotoModal")) {
        const modalVisor = document.createElement("div");
        modalVisor.id = "rodaxFotoModal";
        modalVisor.className = "fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-[150] flex items-center justify-center p-4 hidden opacity-0 transition-opacity duration-200";
...
// Funciones de control de zoom globales para el Modal flotante
